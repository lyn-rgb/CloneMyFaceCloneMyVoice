...
class WanSelfAttention(nn.Module):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs, x_ip=None, ip_grid_sizes=None, ip_freqs=None, ip_offsets=None):
        r"""
        Args:
            x(Tensor): Shape [B, L, C]
...
class WanAttentionBlock(nn.Module):
    ...
    def init_lora(self, train=False):
        self.self_attn.init_lora(train)
    ...
    def forward(
        ...
        y, y_ip = self.self_attn(
            ...
            x_ip=input_x_ip, ip_grid_sizes=ip_grid_sizes, ip_freqs=ip_freqs, ip_offsets=ip_offsets
        )
        with amp.autocast('cuda', dtype=torch.bfloat16):
            x = x + y * e[2].squeeze(2)
            if y_ip is not None:
...
class Head(nn.Module):
    ...
    def forward(self, x, e, x_ip=None, e_ip=None):
        r"""
        Args:
            x(Tensor): Shape [B, L1, C]
        ...
        if x_ip is not None and e_ip is not None:
            assert e_ip.dtype == torch.bfloat16
            with amp.autocast('cuda', dtype=torch.bfloat16):
                e_ip = (self.modulation.bfloat16().unsqueeze(0) + e_ip.unsqueeze(2)).chunk(2, dim=2)
                x_ip = (self.head(self.norm(x_ip) * (1 + e_ip[1].squeeze(2)) + e_ip[0].squeeze(2)))
        return x, x_ip
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def prepare_transformer_block_kwargs(
        ...
        if x_ip is not None:
            x_ip = [self.patch_embedding(u.unsqueeze(0)) for u in x_ip] ## x_ip is list of [B L' D] or [B C 1 H' W']
