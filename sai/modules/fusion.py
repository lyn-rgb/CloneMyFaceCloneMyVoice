...
class FusionModel(nn.Module):
    ...
    def init_lora(self, train=True):
        if self.video_model is not None:
            self.video_model.init_lora(train)
        if self.audio_model is not None:
            self.audio_model.init_lora(train)
    ...
    def single_fusion_block_forward(self,
        ...
        audio_y, audio_y_ip = audio_block.self_attn(
            ...
            input_audio_ip, audio_ip_grid_sizes, audio_ip_freqs, audio_ip_offsets
        )
        with torch.amp.autocast('cuda', dtype=torch.bfloat16):
        ...
        vid_y, vid_y_ip = vid_block.self_attn(
            ...
            input_vid_ip, vid_ip_grid_sizes, vid_ip_freqs, vid_ip_offsets
        )
    ...
    def forward(
        ...
        vid, vid_e, vid_ip, vid_e_ip, vid_kwargs = self.video_model.prepare_transformer_block_kwargs(
            ...
            clip_fea=clip_fea, y=y, first_frame_is_clean=first_frame_is_clean, x_ip=vid_ip
        )

        audio, audio_e, audio_ip, audio_e_ip, audio_kwargs = self.audio_model.prepare_transformer_block_kwargs(
            x=audio, t=t, context=audio_context, seq_len=audio_seq_len, 
            clip_fea=clip_fea_audio, y=None, first_frame_is_clean=False, x_ip=audio_ip
        )
